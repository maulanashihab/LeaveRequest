...
namespace LeaveRequest.Repositories
{
    public class AvailableLeaveRepository : IAvailableLeaveRepository
    {
        ...
        {
            var get = applicationContext.AvailableLeaves.Where(x => (x.Id.ToString().Contains(value) || x.ThisYear.ToString().Contains(value) || x.LastYear.ToString().Contains(value)) && x.IsDelete == false).ToList();
            return get;
        }
